...
class SwitchCHTest {
    ...
    private final SwitchCH provider = new SwitchCH(Mockito.mock(CircuitBreakerService.class));

    @Test
    void getDisplayNameReturnsSwitchCh() {
        Assertions.assertThat(provider.getDisplayName()).isEqualTo("Switch.ch");
    }

    @Test
    void getEndpointNameReturnsSwitchCh() {
        Assertions.assertThat(provider.getEndpointName()).isEqualTo("switch-ch");
    }
    ...
    void getApiUrlReturnsExpectedUrl() {
        Assertions.assertThat(provider.getApiUrl()).isEqualTo("https://dns.switch.ch/dns-query?dns=");
    }
    ...
    @Test
    void interpretReturnsMaliciousWhenCnameMatchesBlockedTargetCaseInsensitively() {
        byte[] message = DNSUtilTest.messageWithAnswers("example.com", List.of(
                new DNSUtilTest.Answer(DNSRecord.CNAME, 60, DNSUtilTest.encodeName("LandingPage.PH.RPZ.Switch.CH"))
        ));
    ...
    @Test
    void interpretReturnsAllowedWhenCnameIsUnrelated() {
        byte[] message = DNSUtilTest.messageWithAnswers("example.com", List.of(
                new DNSUtilTest.Answer(DNSRecord.CNAME, 60, DNSUtilTest.encodeName("other.example.com"))
        ));
    ...
    @Test
    void interpretReturnsAllowedWhenAnswerIsNotACname() {
        byte[] message = DNSUtilTest.messageWithAnswers("example.com", List.of(
