...
class ControlDTest {
    ...
    private final ControlD provider = new ControlD(Mockito.mock(CircuitBreakerService.class));

    @Test
    void getDisplayNameReturnsControlD() {
        Assertions.assertThat(provider.getDisplayName()).isEqualTo("Control D");
    }

    @Test
    void getEndpointNameReturnsControlD() {
        Assertions.assertThat(provider.getEndpointName()).isEqualTo("control-d");
    }
    ...
    void getApiUrlReturnsExpectedUrl() {
        Assertions.assertThat(provider.getApiUrl()).isEqualTo("https://freedns.controld.com/no-malware-typo?name=");
    }

    @Test
    void getDnsFormatReturnsNameMessage() {
        Assertions.assertThat(provider.getDnsFormat()).isEqualTo(DNSFormat.NAME_MESSAGE);
    }
    ...
    @Test
    void interpretReturnsMaliciousWhenAnswerMatchesBlockIp() {
        byte[] message = DNSUtilTest.messageWithAnswers("example.com", List.of(
                new DNSUtilTest.Answer(DNSRecord.A, 60, DNSUtilTest.ipv4("0.0.0.0"))
        ));
